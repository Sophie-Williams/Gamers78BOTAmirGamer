...
client.user.setGame(`Gamers78 Server`,"http://twitch.tv/S-F")
...
client.on('message', msg => {
  if (msg.content === '*link') {
    msg.reply('https://discord.gg/pY9f99j');
  }
});
...
.addField('     ** الوان ** ' ,' ** يظهرلك قائمة الالوان  ** ')
.addField('     ** #color <رقم اللون> ** ' ,' ** لتغير اللون  ** ')
.addField('     ** *link ** ' ,' ** للحصول على رابط السيرفر  ** ')
.addField('     ** ** ' ,' ** أوآمر الإدآرة ** ')
.addField('     ** *bc ** ' ,' ** للبرودكاست ** ')
.addField('     ** *cc** ' , '**لـ مسح الشات** ')
.addField('     ** *ban <name> ** ' ,' ** لاعطاء باند ** ')
...
client.on('message', message => {
  ...
  if(!reason) return message.channel.send(`* *ban @name <Time>*`);
...
client.on('message', message => {
  ...
  let user = message.mentions.users.first();
  let reason = message.content.split(" ").slice(2).join(" ");
  if (message.mentions.users.size < 1) return message.reply("** *kick @name <reson> **");
  if(!reason) return message.reply ("** *kick @name <reson> **");
  if (!message.guild.member(user)
...
client.login("NDk1MTU2NTk2ODM0NTY2MTQ0.DxkW2Q.Yy3WeftW-dfR40zemZevxjadEHQ");
